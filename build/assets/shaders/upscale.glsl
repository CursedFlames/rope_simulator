...
// TODO put this in a lib somewhere, and get a precompiler
...
void main() {
	...
	// currently only blending vertically, since it looks nicer... I think
	...
	// a += 1.0 - clamp((1.0 - fract(pixel)) * scaleFactor.xy, 0.0, 1.0);
...
/*void main() {
	...
	uv += 1.0 - clamp((1.0 - fract(pixel)) * scaleFactor.xy, 0.0, 1.0);
	gl_FragColor = texture2D(uSampler, uv*inputPixel.zw);
}*//*void main() {
	vec2 pixel = vTextureCoord*inputPixel.xy*scaleFactor.zw;
	vec2 uv = floor(pixel) + 0.5;
	uv += 1.0 - clamp((1.0 - fract(pixel)) * scaleFactor.xy, 0.0, 1.0);
